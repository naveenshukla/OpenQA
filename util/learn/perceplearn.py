...
def	update_g_hash(lines, g_hash):
		correct_guesses = 0
		total_lines = 0
		for line in lines:

			(g_hash, accuracy_adddendum) = update_g_hash_with_a_file(line[:line.find(' ')].strip(), line[line.find(' ')+1:].strip(), g_hash)		
...
def	main():
		args = sys.argv[1:]
		if len(args) < 2:
			print('usage: ./perceplearn.py <training_file> <mode_file> -i itrs')
			sys.exit(0)		
		print('preprocessing...')
		...
		f = open(args[0], 'rU', errors = 'ignore')
		lines = f.readlines()
		f.close()

		if len(args) > 2 and args[2] and args[3]:
			itrs = int(args[3])
		else:
			itrs = 20
		...
		for i in range(itrs):
			...
			(g_hash, correct_guesses, total_lines) = update_g_hash(lines, g_hash)
			print(str(i+1)+' ... accuracy '+str(correct_guesses/total_lines))
			if correct_guesses/total_lines > .9999999:
				break
